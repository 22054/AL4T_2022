...
public class InputManagerTest {
    ...
    void arrowKeysMapToActionsDependingOnStatus() {
        ...
        input.keyPressed(new KeyEvent(new java.awt.Canvas(), 0, 0, 0, KeyEvent.VK_UP, ' '));
        verify(engine).receiveInput(ButtonAction.GO_UP);

        reset(engine);
        when(engine.getGameStatus()).thenReturn(GameStatus.MAP_SELECTION);
        input.keyPressed(new KeyEvent(new java.awt.Canvas(), 0, 0, 0, KeyEvent.VK_DOWN, ' '));
        ...
        reset(engine);
        when(engine.getGameStatus()).thenReturn(GameStatus.RUNNING);
        input.keyPressed(new KeyEvent(new java.awt.Canvas(), 0, 0, 0, KeyEvent.VK_RIGHT, ' '));
        verify(engine).receiveInput(ButtonAction.M_RIGHT);

        reset(engine);
        when(engine.getGameStatus()).thenReturn(GameStatus.RUNNING);
        input.keyPressed(new KeyEvent(new java.awt.Canvas(), 0, 0, 0, KeyEvent.VK_LEFT, ' '));
        verify(engine).receiveInput(ButtonAction.M_LEFT);

        reset(engine);
        when(engine.getGameStatus()).thenReturn(GameStatus.RUNNING);
        input.keyPressed(new KeyEvent(new java.awt.Canvas(), 0, 0, 0, KeyEvent.VK_ENTER, ' '));
        verify(engine).receiveInput(ButtonAction.SELECT);

        reset(engine);
        when(engine.getGameStatus()).thenReturn(GameStatus.RUNNING);
        input.keyPressed(new KeyEvent(new java.awt.Canvas(), 0, 0, 0, KeyEvent.VK_ESCAPE, ' '));
    ...
    void spaceFires() {
        ...
        InputManager input = new InputManager(engine);
        when(engine.getGameStatus()).thenReturn(GameStatus.RUNNING);
        input.keyPressed(new KeyEvent(new java.awt.Canvas(), 0, 0, 0, KeyEvent.VK_SPACE, ' '));
    ...
    void mousePressedSelectsMapInSelectionScreen() {
        ...
        input.mousePressed(new MouseEvent(new java.awt.Canvas(), 0, 0, 0, 10, 10, 1, false));
